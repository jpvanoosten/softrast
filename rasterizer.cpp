...
void Mesh::Render(mat4& transform)
{
    ...
    // Compute the 8 corners of the AABB for the mesh in view space.
    for (int i = 0; i < 8; i++)
        c[i] = (transform * vec4(bounds[i & 1].x, bounds[(i >> 1) & 1].y, bounds[i >> 2].z, 1)).xyz;

    // Check if the bounding volume is in the view frustum.
    for (int i, p = 0; p < 5; p++)
    {
        for (i = 0; i < 8; i++)
            if ((dot(Rasterizer::frustum[p].xyz, c[i]) - Rasterizer::frustum[p].w) > 0)
                break;

        // Mesh bounds is outside view frustum.
        if (i == 8) return;
    }

    // transform vertices
    for (int i = 0; i < verts; i++)
        tpos[i] = (transform * vec4(pos[i], 1)).xyz;
    ...
    const int umask = (int)tw - 1;
    const int vmask = (int)th - 1;

    for (int i = 0; i < tris; i++)
    {
        // cull triangle based on the direction of the vertex normal in view space.
        vec3 Nt = (transform * vec4(N[i], 0)).xyz;

        if (dot(tpos[tri[i * 3 + 0]], Nt) > 0)
            continue;
        ...
        int miny = SCRHEIGHT - 1;
        int maxy = 0;
        int h;

        // Prime input vertices.
        for (int v = 0; v < 3; v++)
        {
            cpos[0][v] = tpos[tri[i * 3 + v]];
            cuv[0][v] = uv[tri[i * 3 + v]];
        }
        ...
        for (int p = 0; p < 2; p++)
        {
            for (int v = 0; v < nin; v++)
            {
                ...
                const vec3 A = cpos[from][v];
                const vec3 B = cpos[from][(v + 1) % nin];

                const vec2 Auv = cuv[from][v];
                const vec2 Buv = cuv[from][(v + 1) % nin];

                // Signed distance to the plane.
                const float t1 = dot(plane.xyz, A) - plane.w;
                const float t2 = dot(plane.xyz, B) - plane.w;

                // Case 4: A is not visible, B is visible.
                if ((t1 < 0) && (t2 >= 0))
                {
                    f = t1 / (t1 - t2); // Interpolation factor.

                    cuv[to][nout] = Auv + (Buv - Auv) * f;
                    cpos[to][nout++] = A + f * (B - A);

                    cuv[to][nout] = Buv;
                    cpos[to][nout++] = B;
                }
                // Case 1: Both A and B are on the visible side of the plane.
                else if ((t1 >= 0) && (t2 >= 0))
                {
                    cuv[to][nout] = Buv;
                    cpos[to][nout++] = B;
                }
                // Case 3: A is visible, B is not.
                else if ((t1 >= 0) && (t2 < 0))
                {
                    f = t1 / (t1 - t2);
                ...
                // Case 2: Neither A nor B are on the visible side of the plane. No output.
            }
        ...
        for (int j = 0; j < nin; j++)
        {
            ...
            const float y0 = pos[vert0].y;
            const float y1 = pos[vert1].y;
            const float rydiff = 1.0f / (y1 - y0);

            if ((y0 == y1) || (y0 >= SCRHEIGHT) || (y1 < 1)) 
                continue;

            const int iy0 = max(1, (int)y0 + 1);
            const int iy1 = min(SCRHEIGHT - 2, (int)y1);

            float x0 = pos[vert0].x;
            float dx = (pos[vert1].x - x0) * rydiff;

            float z0 = 1.0f / pos[vert0].z;
            float z1 = 1.0f / pos[vert1].z;
            float dz = (z1 - z0) * rydiff;

            float u0 = tuv[vert0].x * z0;
            float du = (tuv[vert1].x * z1 - u0) * rydiff;

            float v0 = tuv[vert0].y * z0;
            float dv = (tuv[vert1].y * z1 - v0) * rydiff;
        ...
        for (int y = miny; y <= maxy; xleft[y] = SCRWIDTH - 1, xright[y++] = 0)
        {
            float x0 = xleft[y];
            float x1 = xright[y];
            float rxdiff = 1.0f / (x1 - x0);

            float u0 = uleft[y];
            float du = (uright[y] - u0) * rxdiff;

            float v0 = vleft[y];
            float dv = (vright[y] - v0) * rxdiff;

            float z0 = zleft[y];
            float dz = (zright[y] - z0) * rxdiff;

            const int ix0 = (int)x0 + 1;
            const int ix1 = min(SCRWIDTH - 2, (int)x1);
            ...
            for (int x = ix0; x <= ix1; x++, u0 += du, v0 += dv, z0 += dz) // plot span
            {
                ...
                const int u = (int)(u0 * z * tw) & umask;
                const int v = (int)(v0 * z * th) & vmask;

                // Write destination pixel.
                dest[x] = pal[src[u + v * (umask + 1)]];
                // Update depth buffer.
                zbuf[x] = z0;
            }
        }
